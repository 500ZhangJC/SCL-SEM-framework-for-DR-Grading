...
def eval(cfg, model, dataloader, criterion, estimator, device):
    ...
    for step,test_data in tqdm(enumerate(dataloader)):
        ...
        # images_patch = rearrange(X, 'b  c (h p1) (w p2)-> (b h w) c p1 p2', p1=128, p2=128)
        ...
        # images_score = rearrange(images_patch_copy, '(b h w) c p1 p2 -> b c (h p1) (w p2)', b=X.shape[0], h=6,w=6)
