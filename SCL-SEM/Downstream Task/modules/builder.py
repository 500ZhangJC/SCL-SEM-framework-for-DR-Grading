...
def build_model(cfg):
    ...
    if cfg.train.backend == 'torchvision':
...
def build_torchvision_model(network, num_classes, weights=None):
    if weights in (None,False,'null'):
        model = BUILDER[network](weights=None)
    else:
        model = BUILDER[network](weights=models.ResNet50_Weights.DEFAULT)
