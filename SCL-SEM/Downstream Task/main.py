...
def worker(gpu, n_gpus, cfg):
    ...
    # model_lesion = getattr(models, 'resnet50')(pretrained=False, num_classes=1)
    # log_lesion_dir = './resnet50_for_2_fake.pth'
    # if os.path.exists(log_lesion_dir):
    #     checkpoint = torch.load(log_lesion_dir)
    #     model_lesion.load_state_dict(checkpoint['model'],strict=True)
    #     model_lesion.to('cuda')
    #     print('病灶判别网络权重载入成功！')
    # else:
    #     print('病灶判别网络权重载入失败')
    ...
    # checkpoint = os.path.join(cfg.base.save_path,'resnet50_128_08_100.pt')
    # checkpoint = './MIL50_train_fix.pth'
    ...
    # evaluate(cfg, model, model_lesion, test_dataset, estimator)
